...
function ProductCard({ p, index }) {
  ...
  return (
    <div ref={ref} className="reveal hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 group"
      style={{ background: '#fff', borderRadius: '1rem', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ height: '13rem', overflow: 'hidden', position: 'relative', flexShrink: 0 }}>
        ...
        <div style={{ position: 'absolute', bottom: '1rem', left: '1.25rem', right: '1.25rem', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          ...
          <span style={{ color: '#fff', fontWeight: 800, fontSize: '1.25rem' }}>${p.price_per_kg}<span style={{ fontSize: '0.875rem', fontWeight: 400, opacity: 0.75 }}>/kg</span></span>
        </div>
      </div>
      <div style={{ padding: '1.75rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
        <h3 style={{ fontWeight: 700, color: '#111827', marginBottom: '0.75rem', fontSize: '1rem', lineHeight: 1.4 }}>{p.name}</h3>
        <p style={{ color: '#9ca3af', fontSize: '0.875rem', lineHeight: 1.85, marginBottom: '1.5rem', flex: 1 }}>{p.description}</p>
        <div style={{ borderTop: '1px solid #f3f4f6', paddingTop: '1.25rem', marginBottom: '1.5rem' }}>
          {[
            ...
            <div key={l} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.625rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <Link to={`/products/${p.id}`}
            className="flex-1 text-center font-semibold py-3 rounded-full text-sm transition-all duration-300"
  ...
  return (
    <div className="min-h-screen overflow-x-hidden" style={{ background: '#f7f5f0' }}>
      ...
      <section style={{ position: 'relative', color: '#fff', textAlign: 'center', overflow: 'hidden', paddingTop: '9rem', paddingBottom: '5rem' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, maxWidth: '48rem', margin: '0 auto', padding: '0 1.5rem', textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: 'rgba(255,255,255,0.8)', fontSize: '0.875rem', fontWeight: 500, padding: '0.625rem 1.25rem', borderRadius: '9999px', marginBottom: '2rem' }}>
            <span style={{ width: 10, height: 10, borderRadius: '50%', background: '#8dc63f', display: 'inline-block' }} />
            100% Natural & Biodegradable
          </div>
          <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 3.75rem)', textAlign: 'center', marginBottom: '1.25rem', color: '#fff' }}>Our Products</h1>
          <p style={{ color: 'rgba(220,252,231,0.65)', fontSize: '1.125rem', fontWeight: 300, maxWidth: '36rem', margin: '0 auto', lineHeight: 1.85, textAlign: 'center' }}>
      ...
      <div style={{ maxWidth: '72rem', margin: '0 auto', padding: '0 1.5rem 6rem' }}>
        {/* Filter bar */}
        <div style={{ background: '#fff', borderRadius: '1rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', border: '1px solid #f3f4f6', padding: '1.25rem 1.5rem', marginBottom: '2.5rem', display: 'flex', flexWrap: 'wrap', gap: '1.25rem', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ position: 'relative', width: '100%', maxWidth: '20rem' }}>
            ...
            <input type="text" placeholder="Search products…" value={search}
              ...
              style={{ width: '100%', paddingLeft: '2.75rem', paddingRight: '1rem', paddingTop: '0.75rem', paddingBottom: '0.75rem', borderRadius: '0.75rem', border: '1px solid #f3f4f6', background: '#f9fafb', outline: 'none', color: '#1f2937', fontSize: '0.875rem' }} />
          </div>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            {grades.map(g => (
              <button key={g} onClick={() => setGradeFilter(g)}
                ...
                style={gradeFilter === g
                  ? { background: '#39962c', color: '#fff', boxShadow: '0 4px 16px rgba(57,150,44,0.35)', padding: '0.625rem 1.25rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: 600, border: 'none', cursor: 'pointer' }
                  : { background: '#f3f4f6', color: '#6b7280', padding: '0.625rem 1.25rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: 600, border: 'none', cursor: 'pointer' }
                }>{g}</button>
            ))}
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(1, 1fr)', gap: '1.75rem' }} className="md:grid-cols-2 lg:grid-cols-3">
            {filtered.map((p, i) => <ProductCard key={p.id} p={p} index={i} />)}
          </div>
        )}

        {/* Tech specs */}
        <div ref={specsRef} className="reveal" style={{ marginTop: '4rem', borderRadius: '1rem', overflow: 'hidden', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
          <div style={{ position: 'relative', height: '11rem', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', padding: '0 2.5rem', background: 'linear-gradient(135deg,rgba(8,20,9,0.92),rgba(55,89,59,0.88))' }}>
              <div>
                <span style={{ color: '#8dc63f', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', fontSize: '0.75rem' }}>Technical Data</span>
                <h3 style={{ fontSize: '1.875rem', color: '#fff', marginTop: '0.75rem' }}>Technical Specifications</h3>
              </div>
            </div>
          </div>
          <div style={{ padding: '2.5rem', display: 'grid', gridTemplateColumns: '1fr', gap: '2.5rem', alignItems: 'center', background: 'linear-gradient(135deg,#0d2010,#1e3d22)' }} className="md:grid-cols-2">
            <p style={{ color: 'rgba(187,247,208,0.55)', lineHeight: 1.9, fontSize: '0.875rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              {[
                ...
                <div key={l} style={{ borderRadius: '0.75rem', padding: '1.25rem', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.07)' }}>
                  <div style={{ color: '#8dc63f', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>{l}</div>
                  <div style={{ fontWeight: 700, color: '#fff', fontSize: '0.875rem' }}>{v}</div>
                </div>
              ))}
            </div>
          </div>
